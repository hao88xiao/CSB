...
;(function ($) {
	 ...
	 * onLoad:			(Function:null) The callback function used after the modal dialog(iframe) has loaded
	...
	$.modal.defaults = {
		...
		zIndex: 1000,
		close: true,
		closeHTML: '<a class="modalCloseImg" title="¹Ø±Õ"></a>',
	...
	$.modal.impl = {
		...
		init: function (data, options) {
			...
			else if(data.children("iframe").size()>0){
				iframe=data.children("iframe");
			}
			...
			function iframeLoaded(curIframe){
				// bind the close event to any element in iframe with the closeClass class
				$('.' + s.o.closeClass,curIframe.contents().find("body")).bind('click.simplemodal', function (e) {
					e.preventDefault();
					s.close();
				});
				s.o.onLoad.apply(s, [curIframe]);
			}
